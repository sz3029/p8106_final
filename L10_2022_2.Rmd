...
gen_data <- function(N)
{
  ...
  X1 <- X[,1]
  X2 <- exp(X[,2])
  X3 <- factor(c(rep("pos",N/2), rep("neg",N/2)))
  eps <- rnorm(N, sd = .5)
  Y <- X1 + log(X2) + (X3 == "pos") + eps

  ind_missing <- rbinom(N, size = 1, prob = exp(X[,2]/2)/(1+exp(X[,2]/2)))
  
  X1[ind_missing == 1] <- NA
  X3[sample(1:N, N*0.2)] <- NA
  data.frame(Y = Y, X1 = X1, X2 = X2, X3 = X3)
}

set.seed(2022)

trainData <- gen_data(1000)
testData <- gen_data(200)

# description of the preprocessing steps
rec <- recipe(Y ~ ., data = trainData) %>% 
  step_impute_knn(all_predictors(), neighbors = 5) %>% 
  step_BoxCox(X2) %>%
  step_center(all_numeric_predictors()) %>%
  step_scale(all_numeric_predictors())

# the recipe object can be used as the first argument of train()
fit <- train(rec, 
             data = trainData,
             method = "lm",
             trControl = trainControl(method = "cv"))

head(predict(fit, newdata = testData))

# estimate the required parameters from a (training) dataset
...
head(testData[is.na(testData$X1)|is.na(testData$X3),])
head(testData_pp[is.na(testData$X1)|is.na(testData$X3),])
```
